...
const AddUGPROGRAMMES = () => {
    ...
    const onSubmit = async (data) => {
        ...
        const res = await fetcher.post("ug_section", featureData);
        console.log(res);
        reset();
    ...
    const deleteFeature = (id) => {
        ...
        if (proced) {
            ...
            const url = `http://localhost:5000/ug_delete/${id}`;
            fetch(url, {
                method: 'DELETE'
    ...
    const statusChange = async (id, stat) => {
        ...
        const res = await fetcher.put(`ug-status/${id}`, statusData);
        console.log(res)
        // toast('Data Successfully uploaded')
    }
...
export default AddUGPROGRAMMES;